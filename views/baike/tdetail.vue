<template>
  <div id="tdetailer" class="ctdetailer">
    ...
    <div class="tpic_show1"><img :src="img"></div>
    <div class="state">
      <div class="state1">
    ...
    <div class="jiqiao">
      <div class="tpic_show">
        <router-link :to="{path:'/baike/wsjq',query:{data:data.select_skill,name:'挑选技巧',full_name:data.full_name,category:data.category}}"><p>挑选技巧</p></router-link>
      </div>
      <div class="tpic_show">
        <router-link :to="{path:'/baike/wsjq',query:{data:data.feed_ways,name:'喂食方式',full_name:data.full_name,category:data.category}}"><p>喂食方式</p></router-link>
      </div>
    </div>
    <div class="changshi">
      <div class="tpic_show">
        <router-link :to="{path:'/baike/wsjq',query:{data:data.feature,name:'宠物特点',full_name:data.full_name,category:data.category}}"><p>宠物特点</p></router-link>
      </div>
      <div class="tpic_show">
        <router-link :to="{path:'/baike/wsjq',query:{data:data.life_habit,name:'生活习性',full_name:data.full_name,category:data.category}}"><p>生活习性</p></router-link>
      </div>
    </div>
  </div>
</template>
    ...
    $(document).ready(function () {
      $.ajax({
        ...
        success: function (data) {
          that.data = data['data']
          that.img = (that.data.img.split('&'))[0];
          console.log(data)
        },
